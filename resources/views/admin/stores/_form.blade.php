...
<form class="stack" method="POST" action="{{ $action }}" enctype="multipart/form-data">
    ...
    <label>
        ...
        <select name="tenant_id" required>
            ...
            @foreach ($tenants as $tenantOption)
                <option value="{{ $tenantOption->id }}" @selected(old('tenant_id', $store->tenant_id ?? null) == $tenantOption->id)>{{ $tenantOption->name }}</option>
            @endforeach
        </select>
    </label>
    ...
    <div class="two-up">
        ...
        <label>
            Currency
            <input type="text" name="currency" value="{{ old('currency', $store->currency ?? 'USD') }}" maxlength="3" required>
        </label>
    </div>
