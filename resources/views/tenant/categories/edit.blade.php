...
@section('content')
    <section class="panel">
        ...
        <form class="stack" method="POST" action="{{ route('dashboard.categories.update', [$tenant, $category]) }}">
            @csrf
            @method('PUT')
            ...
            <label>
                ...
                <select name="store_id" required>
                    @foreach ($stores as $store)
                        <option value="{{ $store->id }}" @selected(old('store_id', $category->store_id) == $store->id)>{{ $store->name }}</option>
                    @endforeach
                </select>
            </label>
            ...
            <div class="actions">
                ...
                <a class="button secondary" href="{{ route('dashboard.categories.index', $tenant) }}">Back</a>
            </div>
        </form>
    </section>
@endsection
