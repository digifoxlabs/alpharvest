...
<form class="stack" method="POST" action="{{ $action }}" enctype="multipart/form-data" data-category-scope>
        ...
        <label>
            ...
            <select name="store_id" required data-category-store>
                ...
                @foreach ($stores as $store)
                    <option value="{{ $store->id }}" @selected(old('store_id', $product->store_id ?? null) == $store->id)>{{ $store->name }}</option>
                @endforeach
            </select>
        </label>
    ...
    <div class="actions quick-links-row">
        <a class="button secondary" href="{{ route('dashboard.stores.index', $tenant) }}">Create or manage stores</a>
        <a class="button secondary" href="{{ route('dashboard.categories.index', $tenant) }}">Create or manage categories</a>
    </div>
